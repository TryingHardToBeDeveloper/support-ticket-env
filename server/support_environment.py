...
class SupportTicketEnvironment(Environment):
    ...
    def __init__(self) -> None:
        super().__init__()
        self._tickets = load_ticket_bank()
        self._task_id: int = 1
        self._ticket: dict = {}
        self._classified: bool = False
        self._agent_category: str | None = None
        self._classified_correctly: bool = False  # tracks actual correctness, not just attempt
        # Accumulated classification partial credit for Task 2.
        self._task2_cls_score: float = 0.0
    ...
    def get_metadata(self):
        ...
        return EnvironmentMetadata(
            ...
            description=(
                "A customer-support triage environment where an AI agent "
                "classifies tickets, selects actions, and resolves queues."
            ),
            version="1.0.0",
            author="AlgoCore",
    ...
    def step(self, action: SupportAction, **kwargs) -> SupportObservation:  # type: ignore[override]
        ...
        else:  # Defensive: reset validates this invariant.
            raise RuntimeError(f"Unsupported task_id: {self._task_id}")
    ...
    def _step_task1(self, action: SupportAction) -> SupportObservation:
        ...
        self._agent_category = action.category
        ...
        if score == 1.0:
            ...
            feedback = f"❌ Wrong. You said '{action.category}', correct is '{correct}'."
    ...
    def _step_task2(self, action: SupportAction) -> SupportObservation:
        ...
        if not self._classified:
            ...
            cat_score = grade_task1(action.category or "", self._ticket["category"])
            self._classified = True
            self._agent_category = action.category
            self._task2_cls_score = cat_score * 0.3  # store — combined with action score at step 2
            self._classified_correctly = cat_score == 1.0
            return self._make_obs(
                feedback=(
    ...
    def _step_task3(self, action: SupportAction) -> SupportObservation:
        ...
        action_partial = (not action_correct) and pair in {frozenset({"reply", "escalate"})}
        ...
        score = grade_task3(
            ...
            requires_reply=self._ticket["correct_action"] == "reply",
            resolved=True,
            steps_taken=self._step_count,
            max_steps=MAX_STEPS,
        )
        normalized_score = round(score / self._tickets_total, 4)
        ...
        if self._tickets_resolved < self._tickets_total:
            ...
            self._agent_category = None
            feedback = (
                f"Ticket evaluated (score {normalized_score:.2f}). "
            ...
            feedback = (
                ...
                f"Episode score: {self._total_reward + normalized_score:.2f}"
            )
            done = True
            self._resolved = True

        # Task 3's episode return is normalized across the three-ticket queue.
        return self._make_obs(
            feedback=feedback,
    ...
    def _make_obs(
        ...
        return SupportObservation(
            ...
            current_category=self._agent_category if self._classified else None,
