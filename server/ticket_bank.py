"""Server-only evaluation data.

This module MUST NOT be included in the client distribution. Evaluated agents
receive ticket text through observations and never import this answer bank.
...
def load_ticket_bank() -> list[dict]:
    """Load an optional held-out bank mounted into the server deployment."""
    configured_path = os.getenv("SUPPORT_ENV_TICKET_BANK")
    if not configured_path:
        return TICKETS
    path = Path(configured_path)
    data = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(data, list) or len(data) < 3:
        raise ValueError("ticket bank must be a JSON list containing at least three tickets")
    required = {"id", "text", "category", "correct_action", "resolution_hint"}
    ...
    for index, ticket in enumerate(data):
        if not isinstance(ticket, dict) or not required <= ticket.keys():
            raise ValueError(f"ticket bank entry {index} is missing required fields")
        if ticket["id"] in seen_ids:
            raise ValueError(f"duplicate ticket id: {ticket['id']}")
        if ticket["category"] not in valid_categories:
            raise ValueError(f"invalid category in ticket {ticket['id']}")
        if ticket["correct_action"] not in valid_actions:
            raise ValueError(f"invalid action in ticket {ticket['id']}")
